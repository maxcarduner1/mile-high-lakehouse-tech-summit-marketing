 ...
 * OpenAI-compatible interface (configured in campaigndesk.ts → configureAgentsSdk).
 *
 ...
 *     a fetch shim in campaigndesk.ts that captures the body to ctx.modelError.
...
export async function streamAgentTurn(args: {
  ...
  res: Response;
  userEmail: string;
  /** MAS endpoint name. Set this OR `genieSpaceId` (one of the two).
   * The agent registers `ask_data` against whichever backend is configured. */
  masEndpointName: string;
  /** Genie space id. Set this OR `masEndpointName`.
   * The two are mutually-exclusive in the default template. */
  genieSpaceId: string;
  ...
  // Captured by the OpenAI fetch shim in campaigndesk.ts on any non-2xx
  ...
  try {
    ...
    const agentInstance = (global as Record<string, unknown>).agentInstanceDEV as
      | { agent: unknown; tools: unknown[] }
      | undefined;
    if (!agentInstance?.agent) {
      throw new Error('Agent not configured — configureAgentsSdk did not set global.agentInstanceDEV');
    }
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const agent = agentInstance.agent as any;
