...
// RETAINED-BUT-UNUSED empty table — kept only to avoid churning the Drizzle
// migration baseline (the known db:generate re-squash crash). NOTHING reads or
// writes it: since the app now binds Angela's Build-1 PRODUCTION branch, there
// is no `raw_creatives` Delta source to mirror (the boot sync leg was removed —
// see db/sync.ts), and `search_creatives` retrieves directly from her Build-1
// Lakebase Search index `brightwave.campaign_search` (BM25 index
// `brightwave.campaign_search_bm25` over the pre-generated `summary_tsv`) — see
// searchCreatives in queries/campaigns.ts. That index is provisioned OUT-OF-BAND
// on Angela's branch, not by Drizzle: the table is owned by a different
// principal than the app SP, so an SP-run boot migration that issued CREATE
// INDEX would fail with "must be owner". The app SP only reads it.
